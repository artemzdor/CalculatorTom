...
def clear_output_text_ui(window: Window) -> None:
    ...
    for iter_text in [text_percent, text_total_price, text_tax_rate, text_discount, text_base_price, text_state_name]:
...
def write_output_text_ui(transaction: CalculationTransaction, window: Window) -> None:
    ...
    percent_display.update(value=str(transaction.discount_percent.get_percent(base_price=transaction.get_base_price())))
